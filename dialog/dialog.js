	...
	this.show = function (){//id, zindex, withwait, alpha, params){
		...
		if(this.withwait){
			if(this.params == null || this.imgsrc==null){
				...
				var img = 
					"<div style='width: 100%;height: 100%;background: #ffffff61;'><img id='"+this.id+
					"_imgload' src='"+imgsrc+
					"' style='width:11%;position:absolute;top:45%;left:45%'><div>";
			}else{
				var imgsrc = this.imgsrc;
				var img=
					"<img id='"+this.id+
					"_imgload' src='"+imgsrc+
					"' style='width:11%;position:absolute;top:45%;left:45%'>";
			}		
...
storeobj = function(params_){
	this.params = params_;
	this.url = '';//''= lokal //"index.php?Op=9&Pg=kec&prs=getall";
